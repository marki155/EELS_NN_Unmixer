...
from hyperspy._signals.eels_tf import EELSSpectrum_TF
from hyperspy.components1d import EELSCLEdge_TF, PowerLaw_TF
...
class EELSModel_TF(Model1D):
    ...
    def signal1D(self, value):
        if isinstance(value, EELSSpectrum_TF):
            self._signal = value
        else:
            raise ValueError(
    ...
    def append(self, component):
        ...
        if isinstance(component, EELSCLEdge_TF):
    ...
    def _classify_components(self):
        ...
        for component in self:
            ...
            elif (isinstance(component, PowerLaw_TF) or
                  component.isbackground is True):
                self._background_components.append(component)

        if self.edges:
            self.edges.sort(key=EELSCLEdge_TF._onset_energy)
        ...
        if len(self._background_components) > 1:
            ...
            if isinstance(bg, PowerLaw_TF) and self.edges and not \
               bg.A.map["is_set"].any():
    ...
    def _add_edges_from_subshells_names(self, e_shells=None):
        ...
        e_shells.sort()
        master_edge = EELSCLEdge_TF(e_shells.pop(), self.GOS)
        ...
        while len(e_shells) > 0:
            ...
            if next_element != element:
                ...
                # master edge
                edge = EELSCLEdge_TF(e_shells.pop(), GOS=self.GOS)
    ...
    def two_area_background_estimation(self, E1=None, E2=None, powerlaw=None):
        ...
        if powerlaw is None:
            for component in self._active_background_components:
                if isinstance(component, components1d.PowerLaw_TF):
                    if powerlaw is None:
                        powerlaw = component
                    else:
                        _logger.warning(
