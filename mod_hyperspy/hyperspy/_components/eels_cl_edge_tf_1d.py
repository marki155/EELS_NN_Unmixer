...
class EELSCLEdge_TF(Component):
    ...
    def function(self, E, parameter, parameters_fine):
        ...
        """
        knots = tf.convert_to_tensor(self.knots)
        ...
        if self.fine_structure_active is True:
            ...
            app = tf.constant([0,0,0,0], dtype='float64')
            param_f = tf.concat([parameters_fine, app], 0)
            cts_bfs = splev(E[bfs_start:bfs_end], knots, param_f, 3)[:, 0]
            cts = tf.concat([tf.zeros((bfs_start), dtype='float64'), cts_bfs],axis=0)
        else:
            cts = tf.zeros((bfs_end), dtype='float64')
            #bsignal[bfs] = False
        itab_start = bfs_end
        itab_end = np.argmin((E <= Emax))
        if itab_end == 0:
            itab_end = len(E)
        constan = self.tab_xsection(E[itab_start:itab_end])
        tab_reg = tf.convert_to_tensor(constan, dtype='float64')
        cts = tf.concat([cts, tab_reg], axis=0)
        ...
        ending = tf.convert_to_tensor(ending, dtype='float64')
        cts = tf.concat([cts, ending], axis=0)
        #cts[bsignal]
