...
def splev(x, knots, coeffs, degree):
    # Get the shape of the input tensors
    batch_size = tf.shape(x)[0]
    num_knots = tf.shape(knots)[0]

    # Expand the input tensor x to a matrix
    x_matrix = tf.expand_dims(x, axis=1)
    x_matrix = tf.tile(x_matrix, [1, num_knots])

    # Expand the knots tensor to a matrix
    knots_matrix = tf.expand_dims(knots, axis=0)
    knots_matrix = tf.tile(knots_matrix, [batch_size, 1])

    # Calculate the weights for each x value
    weights = tf.math.maximum(0.0, degree - (x_matrix - knots_matrix))
    weights = tf.math.pow(weights, degree)

    # Calculate the dot product of the weights and the coefficients
    result = tf.matmul(weights, tf.expand_dims(coeffs, axis=1))

    return result


@add_gui_method(toolkey="hyperspy.EELSCLEdge_Component")
class EELSCLEdge_TF(Component):
    ...
    def function(self, E, parameter, parameters_fine):
        """Returns the number of counts in barns

        """
        knots = tf.convert_to_tensor(self.knots, dtype='float32')
        ...
        #cts = tf.zeros((len(E)), dtype='float64')

        bfs_start = np.argmax(E >= self.onset_energy.value)
        bfs_end = np.argmin(E < (self.onset_energy.value + self.fine_structure_width))
        ...
        if self.fine_structure_active is True:
            ...
            n_pixel = parameters_fine.shape[0]
            app = tf.zeros((n_pixel,4), dtype='float32')
            param_f = tf.concat([parameters_fine, app], 1)
            cts_bfs_list = []
            for i in range(n_pixel):
                cts_bfs_i = splev(E[bfs_start:bfs_end], knots, param_f[i,:], 3)[:, 0]
                cts_bfs_list.append(cts_bfs_i)
            cts_bfs = tf.stack(cts_bfs_list)
            cts = tf.concat([tf.zeros((n_pixel, bfs_start), dtype='float32'), cts_bfs],axis=1)
        else:
            cts = tf.zeros((n_pixel, bfs_end), dtype='float32')
            #bsignal[bfs] = False
        itab_start = bfs_end
        itab_end = np.argmin((E <= Emax))
        if itab_end == 0:
            itab_end = len(E)
        constan = self.tab_xsection(E[itab_start:itab_end]).astype('float32')
        constan = tf.expand_dims(constan, axis=1)
        constan = tf.tile(constan, [1, n_pixel])
        constan = tf.transpose(constan)
        tab_reg = tf.convert_to_tensor(constan, dtype='float32')
        cts = tf.concat([cts, tab_reg], axis=1)

        #bsignal[itab] = False
        rest_start = np.argmax(E > Emax)
        if rest_start == 0:
            rest_start = len(E)
        ending = self._power_law_A * E[rest_start:] ** -self._power_law_r
        ending = tf.convert_to_tensor(ending, dtype='float32')
        ending = tf.expand_dims(ending, axis=1)
        ending = tf.tile(ending, [1, n_pixel])
        ending = tf.transpose(ending)
        cts = tf.concat([cts, ending], axis=1)
        #cts[bsignal]
        return cts * parameter
